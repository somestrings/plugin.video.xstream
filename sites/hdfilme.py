...
from resources.lib.cloudscraper import CloudScraper
import requests
...
def showEntries(entryUrl=False, sGui=False, sSearchText=False):
    ...
    pattern = '<div[^>]class="listing grid(.*?)<nav'
    isMatch, sContainer = cParser.parseSingleResult(sHtmlContent, pattern)
    if isMatch:
        pattern = '<a[^>]class="block.*?href="([^"]+).*?title="([^"]+).*?data-src="([^"]+)'
